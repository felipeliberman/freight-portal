...
- **Residential detection (two layers, both intended):** the chat agent asks "residence or a business" during quoting and adds RSD that turn — this is deliberate, not a leak, so do not "fix" it out. Geocodio (`zip4.residential === true`) on the delivery street address is the silent backstop that fires at save/dispatch and on form-fill. Ask the customer directly for Canadian addresses (Geocodio is US-only).
