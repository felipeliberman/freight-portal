  ...
  async fetch(request, env) {
    ...
    if (pathname === "/send-confirmation" && request.method === "POST") {
      try {
        ...
        // never received, never paid. The template below stays the truthful one for card payments and
        // for ACH that has actually settled.
        const achInitiated = payload.achStatus === "initiated";
        const invoiceRows = (invoices || []).map(
          (inv) => '<tr style="border-bottom:1px solid #e5e2d9;"><td style="padding:10px 12px;font-size:13px;color:#1a1a1a;">Invoice #' + inv.invNum + '</td><td style="padding:10px 12px;font-size:13px;color:#706c63;">' + (inv.consignee || "N/A") + '</td><td style="padding:10px 12px;font-size:13px;font-weight:600;color:#1a1a1a;text-align:right;">$' + Number(inv.amount || 0).toFixed(2) + "</td></tr>"
        ).join("");
        const paymentDisplay = paymentMethod === "card" ? (cardBrand || "Card") + " ending in " + (cardLast4 || "----") : "ACH Bank Transfer (settles in 2-5 business days)";
        const htmlEmail = '<!DOCTYPE html><html><head><meta charset="UTF-8"></head><body style="font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,sans-serif;color:#1a1a1a;line-height:1.6;margin:0;padding:0;"><table width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;margin:0 auto;padding:20px;"><tr><td style="border-bottom:2px solid #bd27bc;padding-bottom:16px;"><span style="font-size:20px;font-weight:700;color:#bd27bc;">Freight and Logistics, Inc.</span></td></tr><tr><td style="padding:24px 0;"><p>Hi there,</p>' + (achInitiated ? '<p>Your payment has been submitted. Your bank transfer is on its way and typically settles within 2-5 business days.</p><p style="font-size:13px;color:#706c63;">This is a confirmation that we received your payment instruction &mdash; not a receipt of payment. The invoices below stay open until the transfer settles, and we will let you know if anything goes wrong with it.</p>' : '<p>Your payment has been received. Thank you for doing business with us!</p>') + '<p style="font-size:11px;font-weight:600;color:#706c63;text-transform:uppercase;letter-spacing:.05em;">' + (achInitiated ? 'Invoices Submitted for Payment' : 'Invoices Paid') + '</p><table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">' + invoiceRows + '</table><br><table width="100%" cellpadding="0" cellspacing="0" style="background:#f9f8f5;border-radius:8px;"><tr><td style="padding:12px 14px;font-size:13px;color:#706c63;">' + (achInitiated ? 'Amount Submitted' : 'Amount Paid') + '</td><td style="padding:12px 14px;font-size:13px;font-weight:600;color:#1a1a1a;text-align:right;">$' + Number(total || 0).toFixed(2) + '</td></tr><tr><td style="padding:12px 14px;font-size:13px;color:#706c63;">Payment Method</td><td style="padding:12px 14px;font-size:13px;font-weight:600;color:#1a1a1a;text-align:right;">' + paymentDisplay + '</td></tr><tr><td style="padding:12px 14px;font-size:13px;color:#706c63;">Confirmation #</td><td style="padding:12px 14px;font-size:11px;font-family:monospace;color:#706c63;text-align:right;">' + (confirmationId || "N/A") + '</td></tr></table><br><p style="font-size:13px;color:#706c63;">Questions? Email <a href="mailto:support@freightandlogistics.ai" style="color:#bd27bc;">support@freightandlogistics.ai</a> or call <a href="tel:+18006873713" style="color:#bd27bc;">(800) 687-3713</a>.</p></td></tr><tr><td style="border-top:1px solid #e5e5e5;padding-top:16px;font-size:12px;line-height:1.6;color:#8a8a8a;"><div style="font-weight:600;color:#6b6b6b;">Freight and Logistics, Inc. &middot; Nationwide 3PL Freight Brokerage</div><div style="margin-top:2px;"><a href="mailto:support@freightandlogistics.ai" style="color:#bd27bc;text-decoration:none;">support@freightandlogistics.ai</a> &middot; (800) 687-3713 &middot; <a href="https://www.freightandlogistics.ai" style="color:#bd27bc;text-decoration:none;">freightandlogistics.ai</a></div></td></tr></table></body></html>';
        const sendRes = await fetch("https://api.sendgrid.com/v3/mail/send", {
          method: "POST",
          headers: { "Authorization": `Bearer ${SENDGRID_KEY}`, "Content-Type": "application/json" },
          body: JSON.stringify({
            personalizations: [{ to: [{ email: customerEmail }], subject: (achInitiated ? "Payment Submitted \u2014 Freight and Logistics, Inc." : "Payment Confirmation \u2014 Freight and Logistics, Inc.") }],
            from: { email: "support@freightandlogistics.ai", name: "Freight and Logistics" },
